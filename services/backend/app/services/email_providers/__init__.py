"""邮件服务商 provider 工厂：按 provider 名称路由 probe / send 函数。

每个 provider 模块暴露两个函数：
- probe(cfg, secrets) -> None  探活（SMTP NOOP / 云厂商只读 API，不真实发邮件）
- send(cfg, secrets, to, code) -> None  真实发码

失败抛 Exception（由调用方分类转中文错误信息）。
"""

from app.services.email_providers import aliyun_provider, huawei_provider, smtp_provider, tencent_provider
