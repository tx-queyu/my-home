...
@Composable
fun EmailConfigListScreen(
    onBack: () -> Unit,
    onOpenEdit: (String?) -> Unit,
    vm: EmailConfigListViewModel = hiltViewModel(),
) {
    LaunchedEffect(Unit) { vm.refresh() }
    val state by vm.ui.collectAsStateWithLifecycle()
    val snackbarHost = remember { SnackbarHostState() }
    var deleteTarget by remember { mutableStateOf<EmailConfigDto?>(null) }
    ...
    deleteTarget?.let { cfg ->
        AlertDialog(
            ...
            title = { Text("删除配置") },
            text = { Text("确定删除 ${cfg.provider} 邮箱配置？") },
            confirmButton = {
                TextButton(onClick = {
                    val target = cfg
                    deleteTarget = null
...
private fun EmailConfigCard(
    ...
    SettingsCard {
        SettingsRow(
            title = emailProviderLabel(cfg.provider),
            subtitle = buildString {
                append(cfg.fromEmail ?: "（未设置发件人）")
...
internal fun emailProviderLabel(provider: String): String = when (provider) {
    "smtp" -> "SMTP 自建邮箱"
    "aliyun" -> "阿里云邮件"
    "tencent" -> "腾讯云邮件"
    "huawei" -> "华为云邮件"
